...
01. [Javascript API 정리](#01)
    ...
    * [01-16. ``Number값.toFixed()`` - 소수점 계산 오차 보정](#01-16)
...
1.  [알고리즘 정리](#02)
    ...
    * [02-13. ``깊이우선 탐색 (DFS)``](#02-13)
    * [02-14. ``너비우선 탐색 (BFS)``](#02-14)
...
##### 01-16
## 01-16. ``Number값.toFixed()`` - 소수점 계산 오차 보정

프로그래밍 언어를 사용하여 ``소수점 계산`` 을 하면 무한 소수점 ``오차`` 가 발생 합니다.

Javascript 뿐만 아니라, 다른 모든 프로그래밍 언어에서도 발생하는 오차 현상은 ``Number`` 타입의 ``toFixed()`` 메서드를 사용하여 보정할 수 있습니다.

<br/>

``toFixed()`` 의 인자는 보정할 소수점 자리수를 입력하는데, ``0`` 을 인자로 주면, 소수점 1에서 반올림하여 ``소수점이 없는`` 형태로 보정해 줍니다.

<br/>

다음은 ``0.1 + 0.2`` 를 연산한 결과, ``0.3`` 이 아닌 ``무한 소수점 오차`` 가 발생한 예시 입니다.

<img src="./assets/09_toFixed_01.png"><br/>

<br/>

위와 같은 오차를 보정하여 우리가 원하는 값인 ``0.3`` 을 얻기 위해서는 ``toFixed()`` 메서드를 사용할 수 있습니다.

```javascript
const lhs = 0.1;
const rhs = 0.2;

const result = (lhs + rhs).toFixed(1);
...
주의할 점은 ``toFixed()`` 의 반환형은 ``string`` 타입 입니다.
...
``트리`` 또는 ``그래프`` 와 같은 ``비선형 구조`` 데이터를 탐색할 때 사용하는 알고리즘 입니다.

``루트노드`` 에서 시작하여 ``첫번째 하위노드`` 를 탐색하고, 탐색한 경로의 ``종단노드 (Terminal)`` 까지 탐색을 수행합니다.

``종단노드`` 까지 탐색이 완료되면, ``이전 분기 노드`` 에서 다시 ``종단노드``까지 탐색을 수행합니다.

<img src="./assets/11_DFS_01.png"><br/>

<br/>

이와같은 ``깊이우선 탐색 (DFS)`` 는 ``재귀호출`` 의 방식으로 구현하며, 호출되는 재귀함수는 ``Stack`` 자료구조 방식으로 실행되기 때문에, ``Call Stack`` 이라고 합니다. ``(LIFO)``
...
``깊이우선 탐색 (DFS)`` 와 같이, ``트리`` 또는 ``그래프`` 같은 ``비선형 구조`` 데이터를 탐색할 때 사용하는 알고리즘 입니다.

``깊이우선 탐색 (DFS)`` 와 가장 큰 차이점은 탐색 방향에 있습니다.

<br/>

``너비우선 탐색 (BFS)`` 는 ``동일한 Level의 모든 노드`` 를 탐색한 후, 다음 ``Level`` 을 탐색 합니다.

<img src="./assets/12_BFS_01.png"><br/>

위와 같은 동작을 위해서는, ``Queue`` 방식으로 함수를 호출하게 되며, ``FIFO`` 으로 동작합니다.
...
조합은 ``순서에 상관없이, 특정개수를 선택하는 방법의 수`` 를 말합니다.

집합 A가 {1, 2, 3} 일 때, 2개를 선택하는 조합을 구하면 다음과 같습니다.
...
따라서, 3개중 2개를 선택하는 ``조합의 수`` 는 ``3개`` 가 됩니다.

<br/>

조합을 표현하는 기호는 ``C`` 를 사용 합니다. (C: ``Combination``)

``C`` 의 좌측 숫자는 ``선택할 수 있는 개수: n`` 을 나타냅니다.

``C`` 의 우측 숫자는 ``한번에 선택할 개수: r`` 을 나타냅니다.
...
다음은 조합의 수를 구하는 예제 입니다.

1. 집합 {1, 2, 3, 4, 5} 에서, 3개를 선택하는 조합의 수는 몇가지 인가?
...
function calcCombination(n, r) {
  // r === 0 이거나 r === n 일때, 조합 개수가 1개씩 증가
  if(r === 0 || r === n) return 1;

  // nCr === n-1Cr-1 + n-1Cr 공식으로 재귀요청
...
``그래프 (Graph)`` 는 서로 다른 지점의 ``경로`` 를 나타낼때 사용하는 도식 입니다.

<img src="./assets/10_graph_01.png"><br/>

<br/>

위 이미지와 같이 그래프는 3가지 형태가 있습니다.

1. ``무방향`` 그래프
    * ``간선`` 이 연결되어 있으면, 어디든 이동할 수 있습니다.
    
2. ``방향`` 그래프
    * ``화살표 방향`` 으로만 이동할 수 있습니다.

3. ``가중치`` 그래프
    * ``화살표 방향`` 으로만 이동할 수 있으며, 간선별 ``이동 비용`` 이 다릅니다.

<br/>

그래프를 표현하기 위한 방법으로는 3가지 방법이 있습니다.

### 1. ``연결선 리스트``

연결선 리스트는 ``출발 정점`` 과 ``도착 정점`` 을 나타내는 방식 입니다.

```bash
[출뱔정점, 도착정점[, 가중치]]
```

``무방향 그래프``를 ``연결선 리스트`` 로 표현하면 다음과 같습니다.
...
만약 ``가중치 그래프`` 를 ``연결선 리스트`` 로 표현하면 다음과 같습니다.
...
``연결설 리스트`` 는 가장 간편하게 만들 수 있지만, 특정 정점의 데이터에 접근하려면, ``브루트 포스 (Brute Force): 완전탐색`` 을 해야하는 단점이 있습니다.

<br/>

### 2. ``인접행렬``

인접 행렬은 ``출발 정범`` 과 ``도착 정점`` 을 ``2차 배열`` 로 나타내는 방법 입니다.

```javascript
const matrix = [출발정점 번호, 도착정점 번호] = 가중치
```

<br/>

``무방향 그래프`` 를 ``인접행렬`` 로 나타내면 다음과 같습니다.
...
위에 나타낸 ``인접행렬`` 은 다음과 같이 해석할 수 있습니다.
...
```가중치 그래프`` 를 ``인접행렬`` 로 나타내면 다음고 같습니다.
...
``인접행렬`` 은 연결되지 않은 정점까지 모두 표현해야 하기 때문에, ``메모리를 많이 차지`` 하고 시간복잡도가 ``n^2`` 으로 정점이 많을수록 ``느립니다.``

<br/>

### 3. ``인접 리스트``

``인접행렬`` 은 모든 정점을 표현했다면, ``인접 리스트`` 는 각 정점이 ``연결된 노드만`` 나타내는 방식 입니다.

따라서, ``인접행렬`` 에 비해 메모리를 적게 차지하고, 시간복잡도가 ``2n`` 으로 ``빠릅니다.``

<br/>

``무방향 그래프`` 를 ``인접 리스트`` 로 표현하면 다음과 같습니다.

```javascript
matrix[출발정점] = [도착정점A, 도착정점B, ...];
```
...
``가중치 그래프`` 를 ``인접 리스트`` 로 표현하면 다음과 같습니다.

```javascript
matrix[출발정점] = [도착정점A, 가중치A, 도착정점B, 가중치B, ...];
```
...
``인접행렬`` 이나 ``인접 리스트`` 를 사용할 때, ``index`` 번호를 정점번호와 맞추기 위해, ``0번 index`` 는 사용하지 않는 방법도 있습니다.
